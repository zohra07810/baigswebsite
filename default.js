// defaultMarkers.js
export async function loadDefaultMarker(markerEntity, contentDiv, markerData) {
  let currentModel = null;
  const preloadedModels = {};

  // ----------------------------
  // Show model description only
  // ----------------------------
  function showModelDescription(modelData) {
  // Remove old description container if exists
  ...
  if (modelData.description) {
    ...
    desc.style.whiteSpace = "pre-line";  // respect \n for line breaks
    desc.style.wordWrap = "break-word";  // break long words
    desc.style.maxWidth = "600px";       // allow wider container
  ...
  // ----------------------------
  // Load 3D model with loader
  // ----------------------------
  function loadModel(modelData) {
    const modelKey = markerEntity.dataset.id || `marker${markerEntity.dataset.index}`;

    // Hide previous model if any
    Object.values(preloadedModels).forEach(m => m.object3D.visible = false);
    ...
    entity.setAttribute('scale', modelData.default?.scale || modelData.scale || '0.2 0.2 0.2');
    entity.setAttribute('position', modelData.default?.position || modelData.position || '0 0 0');
    ...
    loader.load(
      modelData.default?.path || modelData.path,
      gltf => {
        ...
        setTimeout(() => {
          ...
          contentDiv.innerHTML = '';
          showModelDescription(modelData);
          addChangeViewButton();
        }, 400);
      },
      xhr => {
  ...
  // Start loading the default marker
  loadModel(markerData);
